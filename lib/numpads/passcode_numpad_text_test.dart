...
import 'package:ui_kit/numpads/passcode_numpad_text.dart';
...
void main() {
  group('NumPadText create test', () {
    ...
    testWidgets('renders numpad widget', (WidgetTester tester) async {
      await tester.pumpWidget(
          wrapInMaterialApp(PasscodeNumPadText(onChange: onChangeTextField)));
      expect(find.widgetWithIcon(PasscodeNumPadText, Icons.arrow_back),
          findsOneWidget);
    });
  });
  ...
  group('NumPadText input', () {
    ...
    testWidgets('on tap key tests', (WidgetTester tester) async {
      final testNumPad =
          PasscodeNumPadText(onChange: onChange, textLengthLimit: 4);
    ...
    testWidgets('shows secondary action when no text has been entered',
        (WidgetTester tester) async {
      const secondaryWidget =
          Text('Secondary Action', key: Key('secondaryActionWidget'));
